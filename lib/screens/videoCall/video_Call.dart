...
class _VideoCallState extends State<VideoCall> {
  ...
  Future<void> agoraTokenGenerateDoctor() async {
    final String channel = "call_${widget.id}";
    log("Requesting Agora token for channel: $channel");
    try {
      ...
      log("Agora token generation error: $error\n$stacktrace");
      OslerToast.error(context, "Failed to call the patient! Unable to connect!");
      Navigator.pop(context);
    }
  }
  ...
  Widget _remoteVideo() {
    // FIX: Check that channelName is not null before attempting to render remote video
    if (_remoteUid != null && channelName != null) {
      return AgoraVideoView(
